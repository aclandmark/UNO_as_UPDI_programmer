...
/*
Prog control waits in this subroutine while data is downloaded and takes new commands
from the array store as soon as they become available.

Interrupts are enabled when this sbroutine is running 
...
cli();										//Pause interrrupts between UART_Tx & Rx pair 																							
if(!(High_byte))
{UART_Tx_upload((*cmd_buffer) >> 8);
High_byte = 1;}
else{UART_Tx_upload(*cmd_buffer);
cmd_buffer++;
High_byte = 0; prog_counter += 1;}
UART_Rx_upload();
sei();
//while (r_pointer == w_pointer);	//waiting for new command
