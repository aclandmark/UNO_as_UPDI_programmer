...
newline();
sendChar(record_type + '0');
newline();
sendChar(orphan + '0');
newline();
sendChar(Flash_flag + '0');


Program_end_of_section();
...
if(record_type_old)													//Last record if long enough spans two pages (excludes :00000001FF)
{ 

if((!(Flash_flag)) && (orphan))									//Nothing to save to flash. The last record is a short one.
{sendChar('A');break_page_fill;	}									//Exit UPDI repeat comand early since there is no data to occupy a page


if((Flash_flag) && (orphan)){sendChar('B');						//Data to save to flash. Last line occupies 2 pages
break_page_fill;
write_page_SUB(page_address + 0x20);}								//Write the final partially full page


if((Flash_flag) && (!(orphan))){sendChar('C');					//Data to save to flash
break_page_fill;
write_page_SUB(page_address);}}										//Last line only occupies 1 page


if(!(record_type_old)){											//Hex file has no discontinuities but last line is a short one
if(Flash_flag) {sendChar('D');
break_page_fill;
