...
unsigned int read_flash(int flash_add){							//Read integer from flash
  unsigned int Hexcmd;
UART_Tx_Ls(0x55);											//SYNCH pulses
UART_Tx_Ls(LDS_from | word_address);											//Send command LDS byte
UART_Tx_Ls(flash_add);										//Destination addess LSB
UART_Tx_Ls(flash_add >> 8);								//Destination address MSB
 Hexcmd = UART_Rx_Ls();
 Hexcmd = Hexcmd << 8;
 ...
 UART_Tx_Ls(0x55);											//SYNCH pulses
UART_Tx_Ls(LDS_from | word_address);											//Send command LDS byte
UART_Tx_Ls(flash_add);
UART_Tx_Ls(flash_add >> 8);
Hexcmd |= UART_Rx_Ls();									//Combine both data bytes
