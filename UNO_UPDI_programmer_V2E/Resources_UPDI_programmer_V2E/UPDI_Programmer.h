...
delay_of_0_25uS;\
DDRC &= (~(1 << DDC0));\
while(!(PINC & (1 << PINC0)))test += 1;\
Timer_T0_sub(T0_delay_200us);\
Timer_T0_sub(T0_delay_200us);


/************************************************************************************************************************************/
#define set_UPDI_clock_and_guard_band \
UART_Tx_Ls(0x55);\
UART_Tx_Ls (STCS | 0x09);\
UART_Tx_Ls (0xFD);\
Timer_T0_sub(T0_delay_200us);\
UART_Tx_Ls(0x55);\
UART_Tx_Ls (STCS | 0x02);\
UART_Tx_Ls (0x46);\
Timer_T0_sub(T0_delay_200us);
...
UART_Tx_Ls(0x55);\
UART_Tx_Ls (download_SIB);\
for(int m = 0; m <= 15; m++){\
SIB_byte[m] = UART_Rx_Ls(); }\
newline();\
for(int m = 0; m <= 15;m++){\
sendChar(SIB_byte[m]);}

//sendChar('\t');sendHex(10, P_counter);



#define Initialise_NVM_Programming \
sendString("\r\nInitialising NVM programming. Erase chip? Y or N");\
if (waitforkeypress() == 'Y'){Erase_code();}\
if (set_up_NVM_prog());\
